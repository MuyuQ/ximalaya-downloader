...
/**
 * 生成带序号的文件名
 * @description 序号按指定宽度补零（默认 2 位），保证文件管理器中的正确排序。
 *   超过宽度的序号自动使用实际位数（如 100 集以上的专辑）。
 * @param {string} name - 基础文件名
 * @param {string} ext - 扩展名
 * @param {number} number - 序号（从 1 开始）
 * @param {number} [padWidth=2] - 序号补零宽度
 * @returns {string} 带序号的文件名
 *
 * @example
 * generateNumberedFilename('第一集', '.mp3', 1);      // '01 第一集.mp3'
 * generateNumberedFilename('第一集', '.mp3', 100);    // '100 第一集.mp3'
 * generateNumberedFilename('第一集', '.mp3', 5, 3);   // '005 第一集.mp3'
 */
export function generateNumberedFilename(name, ext, number, padWidth = 2) {
  const safeName = typeof name === 'string' ? name : 'file';
  const safeExt = typeof ext === 'string' ? ext : '';
  const safeNum = typeof number === 'number' && number > 0 ? number : 0;
  const width = typeof padWidth === 'number' && padWidth > 0 ? padWidth : 2;
  const paddedNum = String(safeNum).padStart(width, '0');
  return `${paddedNum} ${safeName}${safeExt}`;
}

/**
 * 检查目录是否可写（不存在时尝试创建）
 * @description 现代化的路径校验：自动创建缺失目录，并实际写入探测文件验证权限。
 * @param {string} dirPath - 目录路径
 * @returns {Promise<boolean>} 可写返回 true
 *
 * @example
 * if (!(await ensureWritableDirectory('./downloads'))) {
 *   console.error('下载路径不可用');
 * }
 */
